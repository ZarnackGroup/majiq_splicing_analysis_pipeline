...
class RowChecker:
    ...
    def validate_and_transform(self, row):
        ...
        if self._strandedness_col and self._strandedness_col in row:
            self._validate_strandedness(row)  # Only validate if column exists
    ...
    def _validate_second(self, row):
        """Only validate fastq_2 if it exists in the file."""
        if self._second_col and self._second_col in row and row[self._second_col]:
            self._validate_fastq_format(row[self._second_col])


    def _validate_strandedness(self, row):
        """Only validate strandedness if the column exists and has a value."""
        if self._strandedness_col and self._strandedness_col in row and row[self._strandedness_col]:
    ...
    def _validate_pair(self, row):
        """Ensure paired-end consistency and set single-end status."""
        if self._second_col and self._second_col in row and row[self._second_col]:
            # Paired-end sample
            ...
            # Single-end sample (or no fastq_2 column)
...
def check_samplesheet(file_in, file_out):
    ...
    """
    required_columns = {"sample", "fastq_1", "condition"}
    ...
    with file_in.open(newline="") as in_handle:
        ...
        checker = RowChecker(second_col="fastq_2" if has_fastq2 else None)
    ...
    # Write output with correct column headers
    header = ["sample", "single_end", "fastq_1", "condition"]
    if has_fastq2:
        header.append("fastq_2")  # Only include fastq_2 if it exists
