...
- `quality_control/rustc/<SAMPLE>/`
  ...
  - `preseq/`: Library complexity extrapolation output.
  - `dupradar/`: Duplication rate metrics and expression-dependent duplication plots.
  - `rseqc/`: Additional RNA-seq QC metrics generated by RustQC, including BAM statistics, strandedness inference, read distribution, insert-size/inner-distance metrics, junction annotation, junction saturation, duplication, and TIN summaries.
