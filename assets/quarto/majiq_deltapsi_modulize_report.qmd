---
title: "MAJIQ Deltapsi Modulize Report"
...
To avoid unnecessary computations, events with missing values in the columns `event_id`, `gene_id`, `lsv_id`, `gene_name`, `spliced_with_coord` are removed here. After filtering out events with missing values, event types without events left are dropped from the downstream steps.
...
convert_na_probability_to_0 <- function (df) {
  ## ---------------------------------------------
  ## Convert nan and NaN to 0 in probability columns
  ## ---------------------------------------------
...
add_contrast_boolean_calls_majiq <- function(
  ...
  for (i in seq_along(event_list)) {
    ...
    for (ct in contrasts) {
      ...
      # summarize junction information on LSV level to identify LSVs that meet changing criteria
...
```{r signifcant events by contrast}
select_contrast_specific_columns <- function(df, contrast) {

  # Example:
  # contrast = "GBR.YRI"
  # conditions = c("GBR", "YRI")
  contrast_conditions <- strsplit(contrast, "\\.")[[1]]

  # Columns that should remain for this contrast
  keep_cols <- c(
    paste0(contrast_conditions, "_median_psi"),
    paste0(contrast, "_median_dpsi"),
    paste0(contrast, "_probability_changing"),
    paste0(contrast, "_probability_non_changing")
  )

  keep_cols <- intersect(keep_cols, colnames(df))

  # All contrast-/condition-specific columns in the table
  contrast_specific_cols <- grep(
    "(_median_psi|_median_dpsi|_probability_changing|_probability_non_changing|_event_changing)$",
    colnames(df),
    value = TRUE
  )

  # Remove all contrast-specific columns except the ones for this contrast
  drop_cols <- setdiff(contrast_specific_cols, keep_cols)

  df %>%
    select(-any_of(drop_cols))
}
make_significant_events_by_contrast <- function(event_list) {

  all_contrasts <- event_list %>%
    map(~ grep("_event_changing$", colnames(.x), value = TRUE)) %>%
    unlist(use.names = FALSE) %>%
    sub("_event_changing$", "", .) %>%
    unique() %>%
    sort()

  significant_by_contrast <- map(all_contrasts, function(ct) {
    ...
    event_list %>%
      map(function(df) {
        ...
        if (!changing_col %in% colnames(df)) {
          return(df[0, , drop = FALSE])
        }

        changing_event_ids <- df %>%
          filter(.data[[changing_col]]) %>%
          distinct(event_id) %>%
          pull(event_id)

        df %>%
          filter(event_id %in% changing_event_ids) %>%
          select_contrast_specific_columns(ct)
      }) %>%
      discard(~ nrow(.x) == 0)
  })

  names(significant_by_contrast) <- all_contrasts

  significant_by_contrast
}

sanitize_file_names <- function(x) {
...
significant_events_by_contrast <- make_significant_events_by_contrast(annotated_results)
```
...
dir.create(paste0(params$artifact_dir, "/rds/significant_by_contrast"), recursive = TRUE, showWarnings = FALSE)

iwalk(significant_events_by_contrast, function(event_list, contrast) {
  saveRDS(
    event_list,
    file = paste0(
      params$artifact_dir,
      "/rds/significant_by_contrast/",
      sanitize_file_names(contrast),
      "_significant_events.rds"
    )
  )
})
...
write_event_list_xlsx <- function(event_list, outfile) {
  ...
  if (length(event_list) == 0) {
    addWorksheet(wb, "no_significant_events")
    writeData(wb, "no_significant_events", tibble(message = "No significant events for this contrast."))
...
dir.create(
  paste0(params$artifact_dir, "/excel/significant_by_contrast"),
  recursive = TRUE,
  showWarnings = FALSE
)

iwalk(significant_events_by_contrast, function(event_list, contrast) {
  write_event_list_xlsx(
    event_list,
    paste0(
      params$artifact_dir,
      "/excel/significant_by_contrast/",
      sanitize_file_names(contrast),
      "_significant_events.xlsx"
    )
  )
})
...
# Define MultiQC header
mqc_header <- "# id: 'majiq_overview_table'
# section_name: 'MAJIQ'
# description: 'Overview table from MAJIQ Deltapsi modulize analysis results'
# plot_type: 'table'
# pconfig:
#     id: 'majiq_overview_table'
#     title: 'MAJIQ Delta PSI Overview'
"

# Write header first
cat(mqc_header, file = paste0(params$artifact_dir, "/overview_table_mqc.tsv"))

# Append the table data
write.table(
  overview_tbl,
  file = paste0(params$artifact_dir, "/overview_table_mqc.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  append = TRUE  # Important: append to the header
)

```

## Export significant events to BED files

For each contrast, a BED file is written that contains the significant events identified for that contrast. These files can be loaded into genome browsers such as [IGV](https://igv.org/) to inspect the genomic region covered by each event.

The BED intervals are derived from the `junction_coord` column. Because one event can contain multiple junction rows, all `junction_coord` values belonging to the same `event_id` are combined, and the exported BED interval spans from the minimum junction coordinate to the maximum junction coordinate of that event.

The BED files use the standard BED6 format:

`chrom`, `chrom_start`, `chrom_end`, `name`, `score`, `strand`

The `chrom_start` coordinate is converted to BED format by subtracting 1 from the minimum junction coordinate, because BED uses 0-based start coordinates. The `chrom_end` coordinate is kept as the maximum junction coordinate. The interval name is constructed from the gene name, event type, and event ID, for example:

`TPM1|cassette|event_42`


```{r export bed files}
make_event_bed_from_event_list <- function(event_list) {

  imap_dfr(event_list, function(df, event_type) {
    ...
    df %>%
      ...
      mutate(
        junction_coord = as.character(junction_coord),
        coord_range = str_extract_all(junction_coord, "\\d+\\s*-\\s*\\d+")
      ) %>%
      unnest_longer(coord_range, values_to = "coord_range") %>%
      filter(!is.na(coord_range)) %>%
      separate(
        coord_range,
        into = c("coord_start", "coord_end"),
        sep = "\\s*-\\s*",
        convert = TRUE
      ) %>%
      mutate(
        coord_min = pmin(coord_start, coord_end),
        coord_max = pmax(coord_start, coord_end)
      ) %>%
      group_by(event_id, gene_id, gene_name, seqid, strand) %>%
      summarise(
        chrom_start = min(coord_min, na.rm = TRUE) - 1,
        chrom_end = max(coord_max, na.rm = TRUE),
        .groups = "drop"
      ) %>%
      mutate(
        name = paste(event_type, gene_name, event_id, sep = "|"),
        score = 0
      ) %>%
      select(
        chrom = seqid,
    ...
    distinct() %>%
    arrange(chrom, chrom_start, chrom_end)
}
```

```{r save bed}
dir.create(
  paste0(params$artifact_dir, "/bed/"),
  recursive = TRUE,
  showWarnings = FALSE
)
...
iwalk(significant_events_by_contrast, function(event_list, contrast) {
  ...
  bed_tbl <- make_event_bed_from_event_list(event_list)

  outfile <- paste0(
    params$artifact_dir,
    "/bed/",
    sanitize_file_names(contrast),
    "_significant_events.bed"
  )
