...
The libraries used to create this report. The `tidyverse` package is used to reduce the amount of dependencies. The `openxlsx` package is added on top because `tidyverse` includes no package that can wirte excel sheets. 
...
Summary on the `voila modulize` command used to create the inputs for this report. Thresholds are used later to identify changing and non-changing events between conditions.
...
changing_between_group_dpsi_secondary <-
  suppressWarnings(as.numeric(
    sub(
      paste0(".*--changing-between-group-dpsi-secondary\\s+(", num01, ").*"),
      "\\1",
      command_used
    )
...
Here the actual splicing results are read in.  
...
here "empty" events are removed - if all junctions have no coverage (every psi value is 0), the event is also removed.
...
After cleaning up the tables we annotate each event in every contrast wether it is significantly changing or significantly non-changing. An event is considered significantly changing if it satifies the following criteria:
...
write_event_list_xlsx(annotated_results, "annotated_results.xlsx")
# write_event_list_xlsx(changing_events_only, "regulated_results.xlsx")
...
saveWorkbook(wb_overview, "overview_table.xlsx", overwrite = TRUE)

# ---- 3) save overview table as .tsv ----
write.table(
  overview_tbl,
  file = "overview_table.tsv",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
